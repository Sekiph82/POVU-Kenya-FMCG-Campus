...
`render_m01.py` is the reusable one-video-at-a-time renderer for M01. It imports the preserved REV003 GLB into a temporary Blender process, creates only the requested production camera, and renders 720 PNG frames at 24 fps in local Blender Eevee. `encode_m01.ps1` then encodes those local render frames into the required H.264 MP4 with the installed local ffmpeg.
...
Technical target: 1280x720, 24 fps, frames 0-719, 30 seconds, H.264 MP4, Eevee. Blender 5.2’s headless build cannot assign its exposed FFMPEG image enum, so the lossless-frame plus local-ffmpeg path is intentional.
