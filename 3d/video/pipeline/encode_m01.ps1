...
& ffmpeg.exe -y -hide_banner -loglevel error -framerate 24 -i (Join-Path $framesPath ("frame_%04d.{0}" -f $FrameExtension)) -vf 'scale=1280:720:flags=lanczos' -c:v libx264 -preset medium -crf 18 -pix_fmt yuv420p -movflags +faststart $outputPath
