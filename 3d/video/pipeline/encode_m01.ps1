...
$first = Join-Path $framesPath 'frame_0001.png'
if (-not (Test-Path -LiteralPath $first)) { throw "Missing first rendered frame: $first" }
& ffmpeg.exe -y -hide_banner -loglevel error -framerate 24 -i (Join-Path $framesPath 'frame_%04d.png') -c:v libx264 -preset medium -crf 18 -pix_fmt yuv420p -movflags +faststart $outputPath
